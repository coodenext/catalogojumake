// Corrigindo os IDs dos produtos
const produtos = [
  {
    id: 1,
...
function criarEstrelas(produtoId, container) {
  ...
  for (let i = 1; i <= 5; i++) {
    ...
    estrela.addEventListener("click", (e) => {
      e.stopPropagation()
      const valor = Number.parseInt(e.target.dataset.valor)
      const id = Number.parseInt(e.target.dataset.produtoId)
...
function criarElementoProduto(produto) {
  ...
  if (produto.estoque <= 0) {
    ...
    carousel.appendChild(tagEsgotado)
  }
  ...
  img.src = produto.imagens[currentIndex] || "https://via.placeholder.com/300x300?text=Sem+Imagem"
  ...
  // Adicionar controles de navegação se houver mais de uma imagem
  ...
  // Adiciona a descrição do produto se existir
  ...
  estoque.className = "estoque-contador"
  estoque.textContent = produto.estoque > 0 ? `${produto.estoque} em estoque` : "Indisponível"
  info.appendChild(estoque)

  // Botão WhatsApp
  const botao = document.createElement("a")
  botao.className = `button ${produto.estoque <= 0 ? "disabled" : ""}`

  if (produto.estoque <= 0) {
    botao.textContent = "Indisponível"
    botao.onclick = (e) => e.preventDefault()
  } else {
    botao.innerHTML = '<i class="fab fa-whatsapp"></i> Comprar'
...
function mudarImagemLightbox(direcao) {
  ...
  lightboxImg.src =
    lightboxProdutoAtual.imagens[lightboxImagemAtual] || "https://via.placeholder.com/800x800?text=Sem+Imagem"
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Configurar botão de voltar ao topo
  document.getElementById("btnTopo").addEventListener("click", voltarAoTopo)
